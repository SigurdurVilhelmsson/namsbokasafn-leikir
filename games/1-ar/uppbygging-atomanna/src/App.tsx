...
const STORAGE_KEY = 'uppbygging-atomanna-progress';
...
function App() {
  ...
  const handleLevel1Complete = (score: number, _maxScore: number, hintsUsed: number) => {
    setProgress(prev => ({
      ...prev,
    ...
    trackLevelComplete(1, score, 1200, { hintsUsed });
    setActiveLevel('menu');
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-violet-50 to-purple-100 p-4 md:p-8">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            <h1 className="text-3xl font-bold text-violet-700 mb-2">{t('completion.title')}</h1>
            ...
            <div className="space-y-3 mb-6">
              <div className="bg-violet-50 p-4 rounded-xl flex justify-between items-center">
                <div>
                  <div className="font-bold text-violet-800">{t('levels.level1.name')}</div>
                  <div className="text-sm text-violet-600">{t('levels.level1.description')}</div>
                </div>
                <div className="text-2xl font-bold text-violet-600">{progress.level1Score || 0}</div>
              </div>

              <div className="bg-emerald-50 p-4 rounded-xl flex justify-between items-center">
                <div>
                  <div className="font-bold text-emerald-800">{t('levels.level2.name')}</div>
                  <div className="text-sm text-emerald-600">{t('levels.level2.description')}</div>
                </div>
                <div className="text-2xl font-bold text-emerald-600">{progress.level2Score || 0}</div>
              </div>

              <div className="bg-amber-50 p-4 rounded-xl flex justify-between items-center">
                <div>
                  <div className="font-bold text-amber-800">{t('levels.level3.name')}</div>
                  <div className="text-sm text-amber-600">{t('levels.level3.description')}</div>
                </div>
                <div className="text-2xl font-bold text-amber-600">{progress.level3Score || 0}</div>
              </div>
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className="flex-1 bg-violet-500 hover:bg-violet-600 text-white font-bold py-3 rounded-xl transition-colors"
              >
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h1 className="text-3xl font-bold text-violet-700">{t('game.title')}</h1>
          ...
          {/* How it works */}
          <details className="bg-violet-50 rounded-xl p-4">
            <summary className="font-semibold text-violet-700 cursor-pointer">
              {t('menu.howItWorks')}
            </summary>
        ...
        {/* Atom Overview */}
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          <h2 className="text-lg font-bold text-gray-700 mb-4">Bygging atomsins:</h2>
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3 rounded-xl text-center bg-red-50">
              <span className="text-2xl">&#9881;</span>
              <div className="font-semibold text-sm mt-1 text-red-700">Roteindir (p+)</div>
              <div className="text-xs text-gray-500">I kjarnanum</div>
            </div>
            <div className="p-3 rounded-xl text-center bg-gray-100">
              <span className="text-2xl">&#9898;</span>
              <div className="font-semibold text-sm mt-1 text-gray-700">Nifteindir (n)</div>
              <div className="text-xs text-gray-500">I kjarnanum</div>
            </div>
            <div className="p-3 rounded-xl text-center bg-blue-50">
              <span className="text-2xl">&#9899;</span>
              <div className="font-semibold text-sm mt-1 text-blue-700">Rafeindir (e-)</div>
              <div className="text-xs text-gray-500">Utan um kjarnann</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full p-6 rounded-xl border-4 border-violet-400 bg-violet-50 hover:bg-violet-100 transition-all text-left"
          >
            <div className="flex items-center gap-4">
              <div className="text-4xl">&#9883;</div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-xl font-bold text-violet-800">{t('levels.level1.name')}</span>
                ...
                <div className="text-sm mt-1 text-violet-600">{t('levels.level1.description')}</div>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className={`text-xl font-bold ${isLevel2Unlocked ? 'text-emerald-800' : 'text-gray-500'}`}>
                ...
                <div className={`text-sm mt-1 ${isLevel2Unlocked ? 'text-emerald-600' : 'text-gray-500'}`}>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className={`text-xl font-bold ${isLevel3Unlocked ? 'text-amber-800' : 'text-gray-500'}`}>
                ...
                <div className={`text-sm mt-1 ${isLevel3Unlocked ? 'text-amber-600' : 'text-gray-500'}`}>
