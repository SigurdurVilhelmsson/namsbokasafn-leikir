...
import { ELECTROLYTE_CATEGORIES, ElectrolyteType } from './data/substances';
...
const STORAGE_KEY = 'jonir-i-lausn-progress';
      ...
      <div className="min-h-screen bg-gradient-to-br from-cyan-50 to-blue-100 p-4 md:p-8">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            <h1 className="text-3xl font-bold text-cyan-700 mb-2">{t('completion.title')}</h1>
            ...
            <div className="space-y-3 mb-6">
              <div className="bg-amber-50 p-4 rounded-xl flex justify-between items-center">
                <div>
                  <div className="font-bold text-amber-800">{t('levels.level1.name')}</div>
                  <div className="text-sm text-amber-600">{t('levels.level1.description')}</div>
                </div>
                <div className="text-2xl font-bold text-amber-600">{progress.level1Score || 0}</div>
              </div>

              <div className="bg-blue-50 p-4 rounded-xl flex justify-between items-center">
                <div>
                  <div className="font-bold text-blue-800">{t('levels.level2.name')}</div>
                  <div className="text-sm text-blue-600">{t('levels.level2.description')}</div>
                </div>
                <div className="text-2xl font-bold text-blue-600">{progress.level2Score || 0}</div>
              </div>

              <div className="bg-teal-50 p-4 rounded-xl flex justify-between items-center">
                <div>
                  <div className="font-bold text-teal-800">{t('levels.level3.name')}</div>
                  <div className="text-sm text-teal-600">{t('levels.level3.description')}</div>
                </div>
                <div className="text-2xl font-bold text-teal-600">{progress.level3Score || 0}</div>
              </div>
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className="flex-1 bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-3 rounded-xl transition-colors"
              >
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h1 className="text-3xl font-bold text-cyan-700">{t('game.title')}</h1>
          ...
          {/* How it works */}
          <details className="bg-cyan-50 rounded-xl p-4">
            <summary className="font-semibold text-cyan-700 cursor-pointer">
              {t('menu.howItWorks')}
            </summary>
        ...
        {/* Electrolyte Types Overview */}
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          <h2 className="text-lg font-bold text-gray-700 mb-4">{t('menu.overview')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {(Object.keys(ELECTROLYTE_CATEGORIES) as ElectrolyteType[]).map(cat => (
              <div
                key={cat}
                className="p-3 rounded-xl text-center"
                style={{ backgroundColor: `${ELECTROLYTE_CATEGORIES[cat].color}15` }}
              >
                <span className="text-2xl">{ELECTROLYTE_CATEGORIES[cat].emoji}</span>
                <div className="font-semibold text-sm mt-1" style={{ color: ELECTROLYTE_CATEGORIES[cat].color }}>
                  {ELECTROLYTE_CATEGORIES[cat].name}
                </div>
                <div className="text-xs text-gray-500 mt-1">
                  {ELECTROLYTE_CATEGORIES[cat].conductivity}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full p-6 rounded-xl border-4 border-amber-400 bg-amber-50 hover:bg-amber-100 transition-all text-left"
          >
            <div className="flex items-center gap-4">
              <div className="text-4xl">💡</div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-xl font-bold text-amber-800">{t('levels.level1.name')}</span>
                ...
                <div className="text-sm mt-1 text-amber-600">{t('levels.level1.description')}</div>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className={`text-xl font-bold ${isLevel2Unlocked ? 'text-blue-800' : 'text-gray-500'}`}>
                ...
                <div className={`text-sm mt-1 ${isLevel2Unlocked ? 'text-blue-600' : 'text-gray-500'}`}>
          ...
          >
            <div className="flex items-center gap-4">
              <div className="text-4xl">{isLevel3Unlocked ? '⚗️' : '🔒'}</div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className={`text-xl font-bold ${isLevel3Unlocked ? 'text-teal-800' : 'text-gray-500'}`}>
                ...
                <div className={`text-sm mt-1 ${isLevel3Unlocked ? 'text-teal-600' : 'text-gray-500'}`}>
        ...
        <div className="mt-6 text-center">
          <a href="../" className="text-gray-500 hover:text-cyan-600 text-sm">
