...
const STORAGE_KEY = 'stilltu-efnajofnur-progress';
...
function App() {
  ...
  } = useAchievements({ gameId: 'stilltu-efnajofnur' });
  ...
  const handleLevel3Complete = (
    ...
    setProgress(prev => ({
      ...
      level3BestCorrect: Math.max(prev.level3BestCorrect, correct),
      totalGamesPlayed: prev.totalGamesPlayed + 1,
    }));
          ...
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="flex-1">
              <h1 className="text-4xl font-bold text-center mb-2 text-gray-800">
                {t('game.title', 'Stilltu efnajöfnur')}
              </h1>
              <p className="text-center text-gray-600">
                {t('game.description', 'Lærðu að stilla efnajöfnur')}
              </p>
            </div>
            <AchievementsButton
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <h3 className="text-xl font-bold text-gray-800">
                      {t('level1.title', 'Sjónræn stilling')}
                    </h3>
                  </div>
                  <p className="text-gray-600 text-sm">
                    {t('level1.description', 'Sjáðu atóm sem litaða hringi')}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <h3 className="text-xl font-bold text-gray-800">
                      {t('level2.title', 'Stilling með skoðun')}
                    </h3>
                    {!progress.level1Completed && (
                  ...
                  <p className="text-gray-600 text-sm">
                    {t('level2.description', 'Notaðu vísbendingar og greiningarhæfni')}
                  </p>
                </div>
                <div className="text-right">
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    {t('level3.description', 'Flóknar efnajöfnur og áskorunarhamur')}
                  </p>
                </div>
                <div className="text-right">
                  {progress.level3BestScore > 0 ? (
                    <div className="text-green-600">
                      ...
                      <div className="text-xs">
                        {progress.level3BestCorrect} {t('level3.equationsBalanced', 'jöfnur')}
                      </div>
                    </div>
                  ) : (
                    <div className="text-gray-400 text-3xl">→</div>
        ...
        <div className="bg-orange-50 rounded-xl p-6 mt-6">
          ...
          <ul className="text-sm text-gray-700 space-y-2">
            <li className="flex items-start gap-2">
              ...
              <span>{t('menu.learn1', 'Hvernig á að stilla efnajöfnur')}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-orange-500 mt-0.5">✓</span>
              <span>{t('menu.learn2', 'Skilja lögmál um varðveislu massa')}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-orange-500 mt-0.5">✓</span>
              <span>{t('menu.learn3', 'Þekkja mismunandi gerðir efnahvarfa')}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-orange-500 mt-0.5">✓</span>
              <span>{t('menu.learn4', 'Nota kerfisbundnar aðferðir við flóknar jöfnur')}</span>
            </li>
          </ul>
        </div>
