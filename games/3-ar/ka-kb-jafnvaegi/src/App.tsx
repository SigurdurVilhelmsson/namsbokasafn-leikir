...
const STORAGE_KEY = 'ka-kb-game-progress';

function App() {
  const { updateProgress } = useProgress({
    gameId: 'ka-kb-jafnvaegi',
    initialProgress: {
      currentLevel: 0,
      problemsCompleted: 0,
  ...
  const saveProgress = (newProgress: Partial<GameProgress>) => {
    ...
    const totalProblems =
      (updated.level1Score > 0 ? 10 : 0) +
      (updated.level2Score > 0 ? 8 : 0) +
      (updated.level3Score > 0 ? 8 : 0);
    updateProgress({
      problemsCompleted: totalProblems,
      levelProgress: {
        level1: {
          questionsAnswered: updated.level1Score > 0 ? 10 : 0,
          questionsCorrect: Math.round((10 * updated.level1Score) / 100),
          explanationsProvided: 0,
          explanationScores: [],
          mastered: updated.level1Score >= 80,
        },
        level2: {
          problemsCompleted: updated.level2Score > 0 ? 8 : 0,
          predictionsMade: 0,
          predictionsCorrect: 0,
          finalAnswersCorrect: Math.round((8 * updated.level2Score) / 100),
          mastered: updated.level2Score >= 80,
        },
        level3: {
          problemsCompleted: updated.level3Score > 0 ? 8 : 0,
          compositeScores: [],
          achievements: [],
      ...
      <main id="main-content" className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="text-xl font-bold text-gray-800">
                      {t('menu.level1', 'Þrep 1: Ka/Kb Tjáningar')}
                    </h3>
                  ...
                  <p className="text-gray-600 mt-1">
                    {t('menu.level1Desc', 'Lærðu að skrifa Ka og Kb tjáningar og bera saman styrk')}
                  </p>
                </div>
            ...
            >
              <div className="flex items-center gap-4">
                <div className="text-4xl">{canPlayLevel2 ? '🧮' : '🔒'}</div>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h3
                      className={`text-xl font-bold ${canPlayLevel2 ? 'text-gray-800' : 'text-gray-500'}`}
                    >
                      {t('menu.level2', 'Þrep 2: pH Útreikningar')}
                    </h3>
                  ...
                  <p className={`mt-1 ${canPlayLevel2 ? 'text-gray-600' : 'text-gray-400'}`}>
                    {t('menu.level2Desc', 'Reiknaðu pH veikra sýru- og basalausna')}
                  </p>
                </div>
            ...
            >
              <div className="flex items-center gap-4">
                <div className="text-4xl">{canPlayLevel3 ? '📊' : '🔒'}</div>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h3
                      className={`text-xl font-bold ${canPlayLevel3 ? 'text-gray-800' : 'text-gray-500'}`}
                    >
                      {t('menu.level3', 'Þrep 3: Jónunarprósentu')}
                    </h3>
                  ...
                  <p className={`mt-1 ${canPlayLevel3 ? 'text-gray-600' : 'text-gray-400'}`}>
                    {t('menu.level3Desc', 'Reiknaðu jónunarprósentu og pKa/pKb umbreytingar')}
                  </p>
                </div>
          ...
          <div className="mt-8 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-6">
            <h3 className="text-lg font-bold text-indigo-800 mb-3">
              {language === 'is' ? 'Um sýru-basa jafnvægi' : 'About Acid-Base Equilibrium'}
            </h3>
            <p className="text-gray-700 mb-3">
              {language === 'is'
                ? 'Veikar sýrur og basar sundrast ekki alveg í vatni. Ka og Kb gildin lýsa hversu mikið þau sundrast. Stærra Ka/Kb þýðir sterkara efni.'
                : 'Weak acids and bases do not fully dissociate in water. Ka and Kb values describe how much they dissociate. Larger Ka/Kb means stronger substance.'}
            </p>
            <div className="bg-white rounded-lg p-4 font-mono text-center space-y-2">
              <p>HA(aq) ⇌ H⁺(aq) + A⁻(aq)</p>
              <p>Ka = [H⁺][A⁻]/[HA]</p>
              <p className="text-sm text-gray-500 pt-2">Ka × Kb = Kw = 1.0 × 10⁻¹⁴</p>
            </div>
          </div>
        </div>
      </main>
